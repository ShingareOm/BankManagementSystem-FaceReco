...
void eigenFaceTrainer() {
	...
	// model->save("D:\\trainImage\\recoface-master\\recoface-master\\eigenfaces.yml");
...
void  FaceRecognition() {
	...
	//model->read("D:\\trainImage\\recoface-master\\recoface-master\\eigenfaces.yml");

	Mat testSample = imread("D:\\FaceImages\\0.jpg", 0);
	...
	while (true)
	{
		...
		if (!frame.empty()) {
			...
			/*	model->predict(face_resized, label, confidence);
				...
				putText(original, text, Point(pos_x, pos_y), FONT_HERSHEY_COMPLEX_SMALL, 1.0, CV_RGB(0, 255, 0), 1);
				//cv::imwrite("E:/FDB/"+frameset+".jpg", cropImg);

			}


			putText(original, "Frames: " + frameset, Point(30, 60), FONT_HERSHEY_COMPLEX_SMALL, 1.0, CV_RGB(0, 255, 0), 1);
			putText(original, "No. of Persons detected: " + to_string(faces.size()), Point(30, 90), FONT_HERSHEY_COMPLEX_SMALL, 1.0, CV_RGB(0, 255, 0), 1);
